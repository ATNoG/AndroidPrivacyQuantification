...
def get_api_methods_pii(dir_path, scanner_dir):
    ...
    if os.path.isfile(os.path.join(dir_path, "app_permissions.csv")):
        ...
        app_permissions_levels_piis = [[row[0]] for row in app_permissions_piis_reader]
    ...
    for method in app_methods_file_reader:
        ...
        med = method[1]
        for row in rows:
            if classe == row[1] and med == row[2] and verify_method_has_permission(row[4],app_permissions_levels_piis):
...
# Function to verify if a method has the required permission in the Android Manifest
def verify_method_has_permission(method_perm, list_of_permissions):
    
    if method_perm == '':
        return True
    
    for perm in method_perm.split(","):
        for permiss in list_of_permissions:
            if perm in permiss[0]:
                return True
    
    return False

# Calculate the privacy score(Score Generator)
def calculate_score(dir_path, scanner_dir):
    ...
    # Get methods and permissions relate to privacy
    ...
    if os.path.isfile(os.path.join(dir_path, "app_permissions_piis.csv")):
        app_permissions = open(os.path.join(dir_path, "app_permissions_piis.csv"), 'r', newline='')
        app_permissions_piis_reader = csv.reader(app_permissions, delimiter=';')
        app_permissions_levels_piis = [[row[2], row[0]] for row in app_permissions_piis_reader]
    ...
    if os.path.isfile(os.path.join(dir_path, "app_methods_piis.csv")):
        app_methods = open(os.path.join(dir_path, "app_methods_piis.csv"), 'r', newline='')
        app_methods_pii_reader = csv.reader(app_methods, delimiter=';')
        app_methods_levels_piis = [[row[5], row[2], row[4]] for row in app_methods_pii_reader]
...
# Improvement: Check for duplicated piis
